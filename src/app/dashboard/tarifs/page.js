...
// centimes → euros (affichage)
const toEuros = (cents) => (cents / 100).toString();
            ...
            <div key={t.key} className={styles.priceCard}>
              ...
              <div className={styles.inputEuro}>
                <input
                  ...
                  onChange={(e) => setPonctuel((p) => ({ ...p, [t.key]: e.target.value }))}
                />
            ...
            <div className={styles.aboTable}>
              <div className={styles.aboGrid}>
                ...
                {PLAN_KEYS.map((pk) => {
                  ...
                  return (
                    <>
                      <div key={`${pk.key}-label`} className={styles.aboRowLabel}>
                        {pk.label}
                        <span>{pk.note}</span>
                      </div>
                      <div key={`${pk.key}-once`} className={styles.aboCell}>
                        <input
                          type="number"
                          min="0"
                          step="1"
                          value={once?.price ?? ""}
                          onChange={(e) => setPlanPrice(t.key, pk.key, "ONCE", e.target.value)}
                        />
                        <span className={styles.euro}>€</span>
                      </div>
                      <div key={`${pk.key}-twice`} className={styles.aboCell}>
                        <input
                          type="number"
                          min="0"
                          step="1"
                          value={twice?.price ?? ""}
                          onChange={(e) => setPlanPrice(t.key, pk.key, "TWICE", e.target.value)}
                        />
                        <span className={styles.euro}>€</span>
                      </div>
                    </>
