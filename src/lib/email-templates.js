...
// Lieu des séances (fixe)
const ADRESSE = "303 rue Montagne de Lure, 84260 Sarrians";
const MAPS_URL =
 ...
 */
export function emailConfirmationReservation({ name, formule, dateHeure, duree = "1 heure", lieu = ADRESSE, isEssai = false }) {
  const inner = `
    <tr>
      <td style="padding:10px 44px 0;text-align:center;">
    ...
    <tr>
      <td style="padding:0 40px 8px;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="padding:16px 18px;background:${COLORS.roseBg};border:1px solid ${COLORS.roseBorder};border-radius:10px;">
              <span style="font-size:12px;color:${COLORS.w50};text-transform:uppercase;letter-spacing:0.05em;">Adresse</span><br>
              <span style="font-size:15px;color:${COLORS.rose2};font-weight:700;">${lieu}</span>
            </td>
          </tr>
        </table>
      </td>
    ...
    <tr>
      <td style="padding:22px 44px 8px;text-align:center;">
        <a href="${MAPS_URL}" style="display:inline-block;background:${COLORS.rose};color:${COLORS.white};text-decoration:none;font-weight:700;font-size:14px;padding:13px 30px;border-radius:999px;box-shadow:0 12px 28px rgba(214,76,127,0.4);">Voir l'itinéraire</a>
...
/**
 * Email de rappel de séance (envoyé au client la veille).
 * @param {object} data { name, formule, dateHeure, duree, lieu }
 */
export function emailRappelSeance({ name, formule, dateHeure, duree = "1 heure", lieu = ADRESSE }) {
  const inner = `
    <tr>
      <td style="padding:10px 44px 0;text-align:center;">
        ...
        <h1 style="margin:0 0 12px;font-size:24px;color:${COLORS.white};font-weight:800;">
          C'est <span style="color:${COLORS.rose2};">demain</span> !
        </h1>
        <p style="margin:0 0 26px;font-size:15px;line-height:1.6;color:${COLORS.w60};">
          Bonjour ${name}, un petit rappel pour votre séance à venir. On vous attend !
        </p>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td style="padding:22px 44px 8px;text-align:center;">
        <a href="${MAPS_URL}" style="display:inline-block;background:${COLORS.rose};color:${COLORS.white};text-decoration:none;font-weight:700;font-size:14px;padding:13px 30px;border-radius:999px;box-shadow:0 12px 28px rgba(214,76,127,0.4);">Voir l'itinéraire</a>
        <p style="margin:22px 0 0;font-size:14px;line-height:1.6;color:${COLORS.w50};">
          En cas d'empêchement, prévenez Marie au 06 51 00 14 01. À demain ! 🥋
        </p>
      </td>
    </tr>`;
  return baseLayout(inner);
